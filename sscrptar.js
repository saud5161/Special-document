...
async function loadCommander() {
  try {
    const response = await fetch("commander-‏‏name-ar.json", { cache: 'no-store' });
    ...
    console.error("❌ خطأ في تحميل commander-‏‏name-ar.json:", err);
  }
}
...
const KASHF_FILES = {
"1": {
    "ا": { pdf: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبة ا.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبةا.docm" },
    "ب": { pdf: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبة ب.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبةب.docm" },
    "ج": { pdf: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبة ج.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبةج.docm" },
    "د": { pdf: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبة د.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة1/حضور وانصراف مناوبةد.docm" },
  },
  "5": {
    "ا": { pdf: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبة ا.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبةا.docm" },
    "ب": { pdf: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبة ب.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبةب.docm" },
    "ج": { pdf: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبة ج.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبةج.docm" },
    "د": { pdf: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبة د.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة3/حضور وانصراف مناوبةد.docm" },
  },
  "2": {
    "ا": { pdf: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبة ا.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبةا.docm" },
    "ب": { pdf: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبة ب.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبةب.docm" },
    "ج": { pdf: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبة ج.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبةج.docm" },
    "د": { pdf: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبة د.pdf", doc: "dic-Arrival/الحضور والانصراف/صالة4/حضور وانصراف مناوبةد.docm" },
  }
};
...
} else if (choice === "خطاب-باسم") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/خطاب باسم .docm";
} else if (choice === "خطاب-بدون") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/خطاب بدون اسم.docm";
} else if (choice === "مغادرة-مواطن") {
  wordLink.href = "dic-Arrival/السعودين/مواطن مغادر.docm";
} else if (choice === "تعذر-مغادرة") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/تعذر مغادرة.docm";
} else if (choice === "غياب-افراد") {
  wordLink.href = "dic-Arrival/نماذج الافراد/غياب افراد.docm";
} else if (choice === "عدم-قبول-بصمات") {
  wordLink.href = "dic-Arrival/خطابات جاهزة لتعديل/عدم قبول الخصائص الحيوية.docm";
} else if (choice === "ارتباط-بصمات") {
  wordLink.href = "dic-Arrival/خطابات جاهزة لتعديل/ارتباط بصمات.docm";
} else if (choice === "التزوير") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/تزوير فحص.docm";
} else if (choice === "اشعار") {
  wordLink.href = "dic-Arrival/نماذج الممنوعين/اشعار ترقب.docm";
} else if (choice === "قبض") {
  wordLink.href = "dic-Arrival/نماذج الممنوعين/قبض.docm";
} else if (choice === "اشعار-منع") {
  wordLink.href = "dic-Arrival/نماذج الممنوعين/منع دخول مباحث.docm";
} else if (choice === "عسكري-رحلة-مواصلة") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/رحلة مواصلة عسكري.docm";
} else if (choice === "اشعار-فنانين") {
  wordLink.href = "dic-Arrival/اشعارات/نموذج الاشعارات عن الدبلوماسيين والاعلاميين والفنانين والصحفيين.docm";
} else if (choice === "حذف-السجلات") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/حذف السجلات.docm";
} else if (choice === "اشعار-القوات") {
  wordLink.href = "dic-Arrival/اشعارات/نموذج اشعار باقادمين من منسوبي القوات الأمريكية العسكريين والمدنيين - القوات البريطانية.docm";
} else if (choice === "تاشيرة-دخول") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/نموذج منح تأشيرات دخول.docm";
  } else if (choice === "تاشيرة-ملاح") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/منح تأشيرة ملاحين.docm";
  } else if (choice === "تاشيرة-سياحية") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/خطاب منح تأشيرات سياحة.docm";
} else if (choice === "اشعار-فترة-طويلة") {
  wordLink.href = "dic-Arrival/اشعارات/نموذج الاشعار عن من امضي فترة طويلة خارج المملكة.docm";
} else if (choice === "اشعار-سري") {
  wordLink.href = "dic-Arrival/اشعارات/نموذج اشعار عن الجنسيات المطلوب الاشعار عنها.docm";
} else if (choice === "استلام-اليوم") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/نموذج استلام.docm";
} else if (choice === "تبليغ-مراجعة") {
  wordLink.href = "dic-Arrival/نماذج الممنوعين/مطلوبين تبليغ مراجعة.docm";
} else if (choice === "تطبيق") {
  wordLink.href = "dic-Arrival/نماذج الافراد/تطبيق.docm";
} else if (choice === "صلاحيات") {
  wordLink.href = "dic-Arrival/نماذج الافراد/صلاحيات.docm";
} else if (choice === "منع-دخول") {
  wordLink.href = "dic-Arrival/نماذج الممنوعين/منع دخول جوازات جديد.docm";
} else if (choice === "خطاب-فرد") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/خطاب فرد.docm";
} else if (choice === "خطاب-المناوبات") {
  wordLink.href = "dic-Arrival/نــماذج  اليومية/خطاب مناوبات.docm";
} else if (choice === "مخالفة") {
  wordLink.href = "dic-Arrival/خطوط/مخالفة خطوط.docm";
} else if (choice === "استاذان") {
  wordLink.href = "dic-Arrival/نماذج الافراد/استاذان.docm";
} else if (choice === "منفستات") {
  wordLink.href = "dic-Arrival/خطوط/ملاحظة على المنفست.docm";
} else if (choice === "صلاحيات") {
  wordLink.href = "dic-Arrival/نماذج الافراد/صلاحيات.docm";
  } else if (choice === "انشاء-سجل") {
  wordLink.href = "dic-Arrival/خطابات العرض المتكررة/نموذج بمتابعة عمليات إنشاء السجلات إقامة.docm";
} else {
...
if (choice === "اشعار-القوات") {

  // 1) إخفاء العناصر غير المطلوبة (الآمر المناوب، الهوية، التأشيرة)
  const hideIds = ["commander-name", "commander-rank", , "VisaType"];
  hideIds.forEach(id => {
    const el = document.getElementById(id);
    const lbl = document.querySelector(`label[for='${id}']`);
    if (el) el.style.display = "none";
  ...
  // 2) إخفاء بطاقة "أسماء القوائم والأعمال الإدارية"
  const listCard = document.getElementById("card-lists-admin");
  ...
  // 4) تفعيل وإظهار اسم الفرد ورتبته
  const indName = document.getElementById("IndividualName");
  const indRank = document.getElementById("IndividualRank");
  const indNameLbl = document.querySelector("label[for='IndividualName']");
  const indRankLbl = document.querySelector("label[for='IndividualRank']");

  if (indName) {
    indName.disabled = false;
    indName.style.display = "block";
  }
  if (indNameLbl) indNameLbl.style.display = "block";

  if (indRank) {
    indRank.disabled = false;
    indRank.style.display = "block";
  }
  if (indRankLbl) indRankLbl.style.display = "block";

  // 5) إخفاء خيار "إبقاء النموذج مفتوحًا"
  const keepOpenLabel = document.querySelector(".keep-open-label");
  const keepOpenInput = document.getElementById("KeepPageOpen");
  
  if (keepOpenLabel) keepOpenLabel.style.display = "none";
  if (keepOpenInput) keepOpenInput.style.display = "none";
}
if (choice === "تاشيرة-ملاح"|| choice === "تاشيرة-سياحية") {

  // 1) إخفاء العناصر غير المطلوبة (الآمر المناوب، الهوية، التأشيرة)
  const hideIds = ["commander-name", "commander-rank", "PassportNumber" ,"TravelDestination" , "VisaType"];
  hideIds.forEach(id => {
    const el = document.getElementById(id);
    const lbl = document.querySelector(`label[for='${id}']`);
    if (el) el.style.display = "none";
...
if (choice === "اشعار-سري" || choice === "اشعار-فترة-طويلة") {

  // 1) إخفاء العناصر العامة غير المطلوبة (الآمر المناوب، التأشيرة)
  const hideIds = ["commander-name", "commander-rank", "VisaType"];
  hideIds.forEach(id => {
    const el = document.getElementById(id);
    const lbl = document.querySelector(`label[for='${id}']`);
  ...
  // 3) إظهار بطاقة الفرد + إخفاء عنوانها
  const indivCard = document.getElementById("card-individual");
  if (indivCard) {
    indivCard.style.display = "block"; // إظهار البطاقة
    
    // إخفاء عنوان البطاقة (h3)
    const title = indivCard.querySelector(".card__title");
    if (title) title.style.display = "none";
  }

  // 4) إخفاء اسم الفرد ورتبته
  ["IndividualName", "IndividualRank"].forEach(id => {
    const el = document.getElementById(id);
    const lbl = document.querySelector(`label[for='${id}']`);
    if (el) el.style.display = "none";
  ...
  // 5) إظهار رقم الهوية فقط
  const idInput = document.getElementById("IndividualID");
  // استخدام ID الليبل الموجود في ملف HTML الخاص بك
  const idLabel = document.getElementById("lbl-IndividualID") || document.querySelector("label[for='IndividualID']");

  if (idInput) {
    idInput.style.display = "block";
    idInput.disabled = false; // تفعيل الكتابة
  }
  if (idLabel) idLabel.style.display = "block";

  // 6) إخفاء خيار "إبقاء النموذج مفتوحًا"
...
document.addEventListener("DOMContentLoaded", () => {
  // 1) تحميل قاعدة الأسماء ‏‏name-ar.json
  fetch("‏‏name-ar.json")
    .then(r => r.json())
    .then(db => {
      NAME_DB = db;
    ...
    .catch(err => console.error("فشل تحميل ‏‏name-ar.json", err));
...
async function loadIndividualsWithID() {
  try {
    const res = await fetch("‏‏name-ar.json", { cache: "no-store" });
    const data = await res.json();
